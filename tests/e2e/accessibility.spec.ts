...
for (const locale of ["en", "tr"]) {
  for (const width of [1440, 390]) {
    test(`accessible ${locale} at ${width}px`, async ({ page }) => {
      ...
      await page.getByRole("button", { name: locale === "en" ? "Reduced motion on" : "Azaltılmış hareket açık" }).waitFor();
      ...
      expect(focus.style).toBe("solid"); expect(parseFloat(focus.width)).toBeGreaterThanOrEqual(2);
    });
  }
}
