...
function motionStopped() {
  return (
    ...
    document.querySelector('[data-motion-paused="true"]') !== null
  );
}
export function useProjectCarousel(count: number) {
  ...
  useEffect(() => {
    ...
    let alive = true;
    const sync = () => {
      const focusedSlide = document.activeElement?.closest(
        '[data-project-slide]',
      );
      if (
        focusedSlide &&
        focusedSlide !== api.slideNodes()[api.selectedScrollSnap()]
      ) {
        api
          .rootNode()
    ...
    const settle = () =>
      setState((old) => ({
        ...old,
        settled: api.selectedScrollSnap(),
        announced: deliberate.current,
      }));
    const pointer = () => {
