...
for (const [name, path, width, height] of [
  ['desktop', '/', 1440, 900],
  ['mobile', '/', 390, 844],
  ['desktop-tr', '/tr', 1440, 900],
  ['mobile-tr', '/tr', 390, 844],
  ['tablet', '/', 768, 1024],
  ['small-mobile-tr', '/tr', 360, 800],
]) {
  ...
  await page.addInitScript(() => {
    ...
    new PerformanceObserver((list) => {
      window.__perf.lcp = list.getEntries().at(-1).startTime;
    }).observe({ type: 'largest-contentful-paint', buffered: true });
  });
  await page.goto(
    `${process.env.PREVIEW_URL || 'http://127.0.0.1:3101'}${path}`,
  );
...
console.log(
  records.map(({ name, cls, lcp, overflow, transferBytes, errors }) => ({
    name,
    cls,
    lcp,
    overflow,
