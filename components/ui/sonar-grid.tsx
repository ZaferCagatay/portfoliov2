...
export function SonarGrid({
  ...
  React.useEffect(() => {
    ...
    const cachePoints = () => {
      ...
      for (let i = 0; i < cols; i++)
        for (let j = 0; j < rows; j++)
          points.push([offsetX + i * spacing, offsetY + j * spacing]);
    };
    ...
    const addRing = (x: number, y: number, born: number, ambient = false) => {
      ...
      while (rings.length > Math.max(1, Math.min(12, opts.current.maxRings)))
        rings.shift();
    };
    ...
    const resize = () => {
      ...
      const dpr = Math.min(
        ...
        Math.max(1, Math.min(MAX_DPR, opts.current.maxDpr)),
      );
    ...
    const onDown = (e: PointerEvent) => {
      ...
      if (
        ...
        e.target.closest('a,button,summary,input,textarea,select')
      )
        return;
      const rect = host.getBoundingClientRect();
