...
export function EngineeringStepVisual({ index, labels }: { index: number; labels: readonly string[] }) {
  return (
    <div className={styles.stepVisual}>
      {index === 0 && <div className={styles.scopeFlow}><span>{labels[0]}</span><ArrowRight size={16} aria-hidden="true" /><span>{labels[1]}<ArrowDown size={14} aria-hidden="true" /><strong>{labels[2]}</strong></span></div>}
